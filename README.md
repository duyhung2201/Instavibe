# InstaVibe Ally – AI‑powered social‑planning agents on Google Cloud

Demo video: https://youtu.be/lbMDA5OsSnw

## Overview

InstaVibe Ally is a prototype feature for a social event platform.
It uses multiple specialised agents to discover user interests,
plan tailored outings and publish posts/events back to the
InstaVibe web app.  The system demonstrates how to combine large
language models (LLMs) with tools and external data sources via
open standards while deploying everything to Google Cloud.
...
```text
┌──────────────────────────────┐
│  InstaVibe Web App (Flask)   │
│  • Calls Orchestrator via    │
│    Vertex AI Agent Engine    │
└──────────────┬───────────────┘
               │ A2A / HTTPS
┌──────────────▼───────────────┐
│ Orchestrator Agent           │ Vertex AI Agent Engine
│ • Interprets user intent     │
│ • Delegates tasks via A2A    │
└┬───────────▲───────────┬─────┘
 │           │           │
 │           │           │
▼▼▼         ▼▼▼         ▼▼▼    Cloud Run (one container each)
Planner    Social     Platform
...
Navigate to the `shortcut` directory and run the installation script:
...
Navigate to the `shortcut` directory and run the removal script:
